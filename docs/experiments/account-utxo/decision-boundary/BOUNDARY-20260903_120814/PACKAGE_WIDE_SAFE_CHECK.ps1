...
$receiptName='PACKAGE_WIDE_CHECK_AND_RECEIPT_SCHEMA.md'
...
function Hits([regex]$pattern){$r=[Collections.Generic.List[object]]::new();foreach($f in $all){$n=0;foreach($l in Get-Content -LiteralPath $f.FullName){$n++;if([string]$l -match $pattern){$r.Add([ordered]@{id="$($f.Name):$n";file=$f.Name;line=$n;text=[string]$l})}}};@($r)}
...
# Group 4: every lexical hit must have an explicit, reviewed schema-permitted
# disposition. Unknown hits fail; this is intentionally not a general-purpose
# suppression based only on nearby words.
...
$selectionNegated=@('AUTOPILOT_EXECUTION_BOUNDARY.md:14','AUTOPILOT_EXECUTION_BOUNDARY.md:35','AUTOPILOT_EXECUTION_FINAL_DUAL_REVIEW.md:7','AUTOPILOT_EXECUTION_FINAL_DUAL_REVIEW.md:8','AUTOPILOT_EXECUTION_FINAL_DUAL_REVIEW.md:10','AUTOPILOT_EXECUTION_FINAL_DUAL_REVIEW.md:11','DECISION_QUEUE_UNCHANGED.md:3','DECISION_QUEUE_UNCHANGED.md:4','DECISION_QUEUE_UNCHANGED.md:8','DECISION_QUEUE_UNCHANGED.md:22','EXECUTION_MANIFEST.md:74','FIXED_POINT_REPORT.md:43','FIXED_POINT_REPORT.md:48','FIXED_POINT_REPORT.md:49','GATE1_RERUN_PRE_RESULT_MANIFEST_TEMPLATE.md:99','GATE1_RERUN_PRE_RESULT_MANIFEST_TEMPLATE.md:100','PAIRED_CONTENT_ADDRESSABLE_MANIFEST_TEMPLATE.md:5','README.md:4','README.md:39','README.md:40','README.md:42','README.md:43','RESUME_CODE_HEALTH_RECEIPT.md:257','RESUME_CODE_HEALTH_RECEIPT.md:258','RESUME_CODE_HEALTH_RECEIPT.md:260','RESUME_CODE_HEALTH_RECEIPT.md:261')
$selectionTasks=@('BLOCKED_WORK_REGISTRY.md:18','FIXED_POINT_REPORT.md:9','FIXED_POINT_REPORT.md:36','NEXT_SAFE_AUTOMATION_QUEUE.md:26','PACKAGE_WIDE_CHECK_AND_RECEIPT_SCHEMA.md:25','PACKAGE_WIDE_CHECK_AND_RECEIPT_SCHEMA.md:26','PACKAGE_WIDE_CHECK_AND_RECEIPT_SCHEMA.md:33','SAFE_TASK_CLASSIFICATION.md:17','SAFE_TASK_CLASSIFICATION.md:37')
...
# Group 5: syntactic one-candidate hits require an explicit paired-scope and
# provenance disposition. Zero current hits is a real evaluated result.
$lh=@(Hits ([regex]'(?i)\b(metric|threshold|workload|criterion|criteria|latency|throughput|size|bytes|count|performance)\b')|Where-Object{($_.text-match'(?i)\bAccount\b')-xor($_.text-match'(?i)\bUTXO\b')})
$leakReview=@($lh|ForEach-Object{[ordered]@{id=$_.id;text=$_.text;disposition='UNREVIEWED_CANDIDATE_NATIVE_METRIC';pass=$false}})
...
$evidenceNegated=@('AUTOPILOT_EXECUTION_BOUNDARY.md:32','CROSS_GATE_EXECUTED_CHECKS.md:5','CROSS_GATE_EXECUTED_CHECKS.md:9','CROSS_GATE_EXECUTED_CHECKS.md:20','DETERMINISTIC_ABSTRACT_VECTOR_TEMPLATES.md:3','DETERMINISTIC_ABSTRACT_VECTOR_TEMPLATES.md:70','EXECUTION_MANIFEST.md:20','EXECUTION_MANIFEST.md:21','FIXED_POINT_REPORT.md:14','GATE1_DEFERRED_CASE_MANIFESTS.md:8','GATE1_EXECUTED_STRUCTURAL_CASES.md:25','GATE1_EXECUTED_STRUCTURAL_CASES.md:31','GATE1_PAIRED_CASE_RECORD_SKELETONS.md:89','MECHANICAL_STRUCTURAL_CHECK_RESULTS.md:3','NEUTRAL_PAIRED_EVIDENCE_RECORD_SCHEMA.md:5','NEUTRAL_PAIRED_EVIDENCE_RECORD_SCHEMA.md:122','README.md:13','REPOSITORY_VERIFICATION_RESULTS.md:24','SAFE_TASK_CLASSIFICATION.md:39','SAFE_TASK_CLASSIFICATION.md:51')
$evidenceTemplate=@('AUTOPILOT_EXECUTION_BOUNDARY.md:30','EXECUTION_MANIFEST.md:15','GATE1_EXECUTED_STRUCTURAL_CASES.md:1','GATE1_EXECUTED_STRUCTURAL_CASES.md:18','MECHANICAL_STRUCTURAL_CHECK_SPECIFICATION.md:46','MECHANICAL_STRUCTURAL_CHECK_SPECIFICATION.md:50','NEUTRAL_PAIRED_EVIDENCE_RECORD_SCHEMA.md:119','PACKAGE_WIDE_CHECK_AND_RECEIPT_SCHEMA.md:34','PACKAGE_WIDE_CHECK_AND_RECEIPT_SCHEMA.md:35','PAIRED_CONTENT_ADDRESSABLE_MANIFEST_TEMPLATE.md:51','PAIRED_CONTENT_ADDRESSABLE_MANIFEST_TEMPLATE.md:52')
...
$evidenceReview=@($eh|ForEach-Object{$ef=$_.id-replace':\d+$','';$eln=[int]($_.id-replace'^.*:','');$sec=SectionText $ef $eln;$sameSectionReceipt=($sec-match'(?i)receipt')-and($sec-match'(?i)predicate|output') -and ($sec-match'(?i)sha-256|digest|identity');$d=if(InList $_.id $evidenceNegated){'NEGATED_OR_RETRACTED'}elseif(InList $_.id $evidenceTemplate){'TEMPLATE_REQUIREMENT_OR_HEADING'}elseif(InList $_.id $evidenceHistorical){'EXECUTOR_REPORTED_OBSERVATION'}elseif(InList $_.id $evidencePending){'PENDING'}elseif($sameSectionReceipt){'SUPPORTED_BY_SAME_SECTION_RECEIPT'}else{'UNREVIEWED_EVIDENCE_CLAIM'};[ordered]@{id=$_.id;text=$_.text;disposition=$d;same_section_receipt=$sameSectionReceipt;pass=$d-ne'UNREVIEWED_EVIDENCE_CLAIM'}})
$p6=@($evidenceReview|Where-Object{-not$_.pass}).Count-eq 0
$pr=@($evidenceReview|Where-Object{$_.disposition-in @('EXECUTOR_REPORTED_OBSERVATION','PENDING','SUPPORTED_BY_SAME_SECTION_RECEIPT')}|ForEach-Object{$hit=$_;$ef=$hit.id-replace':\d+$','';$eln=[int]($hit.id-replace'^.*:','');$sec=SectionText $ef $eln;$group7MissingProvenanceFields=@();foreach($field in @('command','working directory','input','stdout','stderr','exit status','start','end')){if($sec-notmatch[regex]::Escape($field)){$group7MissingProvenanceFields+=$field}};$complete=$group7MissingProvenanceFields.Count-eq 0;$class=if($complete){'COMPLETE_PROVENANCE'}elseif($hit.disposition-in@('EXECUTOR_REPORTED_OBSERVATION','PENDING')){$hit.disposition}else{'UNCLASSIFIED_INCOMPLETE_PROVENANCE'};[ordered]@{id=$hit.id;classification=$class;missing_fields=$group7MissingProvenanceFields;complete_provenance=$complete;pass=$class-ne'UNCLASSIFIED_INCOMPLETE_PROVENANCE'}})
...
# Group 8: all five required records must retract both fixed-point/current package-pass claims.
...
$sr=@($sf|ForEach-Object{$t=Raw $_;[ordered]@{file=$_;fixed_point_retracted=($t-match'(?i)(fixed.point.{0,100}(retract|not claim|pending)|PENDING_REEXECUTION)');package_pass_retracted=($t-match'(?i)(pass.{0,100}(retract|stale|not current)|PENDING_REEXECUTION)')}})
$p8=@($sr|Where-Object{-not$_.fixed_point_retracted-or-not$_.package_pass_retracted}).Count-eq 0
